...
describe("serialize", () => {
  ...
  it("sorts nodes by id deterministically", () => {
    const doc: TtDocument = {
      ...
      title: "t",
      nodes: [
        { id: "z", type: "platform", label: "Z", position: { x: 0, y: 0 }, size: { width: 1, height: 1 } },
        { id: "a", type: "enabling", label: "A", position: { x: 0, y: 0 }, size: { width: 1, height: 1 } },
      ],
      interactions: [],
      flows: [],
    };
  ...
  it("produces byte-identical output across runs", () => {
    expect(serializeDocument(SAMPLE_DOCUMENT)).toBe(
      serializeDocument(SAMPLE_DOCUMENT),
    );
  });

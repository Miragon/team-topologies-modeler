...
describe("parseDocument", () => {
  ...
  it("drops legacy v1 edge interactions and the showFlowOfChange flag", () => {
    const result = parseDocument({
      ...
      nodes: [
        { id: "a", type: "platform", label: "A", position: { x: 0, y: 0 }, size: { width: 10, height: 10 } },
      ],
  ...
  it("accepts a placed interaction shape", () => {
    const result = parseDocument({
      ...
      nodes: [],
      interactions: [
        { id: "i1", mode: "collaboration", position: { x: 1, y: 2 }, size: { width: 100, height: 60 } },
      ],
      flows: [],
    });
    expect(result.ok).toBe(true);
  ...
  it("rejects an invalid team type", () => {
    const result = parseDocument({
      ...
      title: "x",
      nodes: [
        { id: "a", type: "not-a-team", label: "A", position: { x: 0, y: 0 }, size: { width: 10, height: 10 } },
      ],
      interactions: [],
      flows: [],
    });
